...
var renameCmd = command.NewCommand(
	...
	command.WithUsageText("qcp rename [old name] [new name]"),
).WithAction(func(v command.Value) error {
	var oldProjectName string
	var newProjectName string
	switch v.NArg() {
	case 0, 1:
		return errors.New("[old name] and [new name] are must, use qcp rename --help to get help")
	case 2:
		oldProjectName = v.Args().Get(0)
		newProjectName = v.Args().Get(1)
	default:
		return errors.New("Args num can not be greater than 2, use qcp rename --help to get help")
	}
